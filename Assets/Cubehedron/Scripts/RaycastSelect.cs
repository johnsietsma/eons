...
[Tooltip( "Casts a Ray against all Colliders in the scene and sends selection events." )]
public class RaycastSelect : FsmStateAction
{
    ...
    public FsmEvent hitStartEvent;

    [Tooltip( "Event to send if the ray hits an object." )]
    [UIHint( UIHint.Variable )]
    public FsmEvent hitEndEvent;
    ...
    void DoRaycast()
    {
        ...
        if ( didHit ) {
            ...
            storeHitObject.Value = hitObject;
        ...
        if ( hitObject != lastHitObject ) {
            if ( lastHitObject != null ) {
                Fsm.Event( hitEndEvent );
            }

            Fsm.Event( hitStartEvent );
        }
